...
% Replace with:
a numerical agreement so precise it left no doubt that the quantisation condition was capturing something real, even without ontological justification.
...
\begin{equation}
  ...
  \label{eq:AL}
\end{equation}
admits \textit{runaway solutions} (acceleration growing exponentially without external force) and \textit{pre-acceleration solutions} (particle responds before the force arrives, violating causality). In QED these pathologies are controlled by renormalisation, which works numerically with extraordinary precision but which Dirac regarded as a fundamental logical inconsistency \cite{dirac_1938_radiating} and Feynman called sweeping the problem under the rug with an elegant technique \cite{feynman_1985_qed}.

\medskip

The ontological question Bohr implicitly raised in 1913---what \textit{kind of thing} is an electron, considered as both a quantum object and a source of its own field?---remains incompletely answered. Each theoretical layer has provided a more adequate partial ontology. None has provided a complete one.

\medskip

This situation is perhaps better understood through the lens of effective field theory \cite{weinberg_1979_eft}: rather than a deficiency awaiting resolution, ontological incompleteness at each energy scale is a permanent structural feature of physical theorising. Each theory carries a partial ontology adequate to its domain; what lies beyond is encoded in the theory's own breakdown, not in a completed picture waiting to be found.

\medskip

\TODO{Extend paragraph on effective field theory (Weinberg, Wilson): EFTs explicitly embrace ontological incompleteness at each scale. That may be a more honest framing than ``the problem is unsolved.''}
