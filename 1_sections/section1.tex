...
We present the calculation in full because the philosophical argument of Chapter [...] depends on it: the failure is not a rough estimate but an exact consequence of three internally consistent and independently verified physical laws.
...
Newton's \emph{Principia Mathematica} \cite{newton_1687_principia} establishes that for uniform circular motion of radius $r$ and speed $v$, differentiating the position vector
$\vec{r}(t) = r(\cos\omega t,\,\sin\omega t)$
twice with $\omega = v/r$ gives
\begin{equation}
...
Measured by torsion balance in 1785 \cite{coulomb_1785_electricity}, the force between two point charges $q_1$ and $q_2$ separated by distance $r$ is
\begin{equation}
  \boxed{F = \frac{1}{4\pi\epsilon_0}\frac{q_1\,q_2}{r^2}},
  \label{eq:coulomb}
\end{equation}
attractive for opposite charges. The constant $\epsilon_0$ is Maxwell's (1865); the inverse-square dependence is Coulomb's direct experimental result.
...
Setting Coulomb's force equal to the centripetal requirement for the electron (mass $m_e$, charge $e$):
\begin{equation}
...
Equating $\dot{E} = -P$ and differentiating~(\ref{eq:energy}):
\begin{equation}
  \frac{e^2}{8\pi\epsilon_0\,r^2}\,\dot{r}
  = -\frac{e^6}{12\pi^3\epsilon_0^3\,m_e^2\,c^3\,r^4}.
\end{equation}
...
Integrating~(\ref{eq:ode}) from $r_0 = a_0$ (the Bohr radius,
$a_0 \approx 0.529\,\text{\AA}$) to $r = 0$:
\begin{equation}
  \int_{a_0}^{0} r^2\,\mathrm{d}r = -C\,t_{\mathrm{collapse}}
  \implies
  \frac{a_0^3}{3} = C\,t_{\mathrm{collapse}},
\end{equation}
giving
\begin{equation}
  \boxed{
    t_{\mathrm{collapse}}
    = \frac{(4\pi\epsilon_0)^2\,m_e^2\,c^3\,a_0^3}{4\,e^4}
...
Sixteen picoseconds. This is not an order-of-magnitude estimate; it is the exact result of integrating three independently verified and internally consistent physical laws. The calculation is impeccable. The conclusion is empirically false. That tension is the starting point of everything that follows.
