...
This state cannot be written as a product $\ket{\phi}_A\otimes\ket{\chi}_B$. Neither particle $A$ nor $B$ has a definite spin potentiality on its own. On the Aristotelian reading, the potentialities are properties not of $A$ and $B$ separately but of the \textit{pair} $AB$ jointly---an irreducibly relational structure.
...
The question arises whether the Aristotelian potentiality reading simply \textit{is} a form of OSR. We suggest it occupies a distinct position. OSR in its strongest form eliminates objects entirely in favour of structures \citep{french_1988_indiscernibles}. The Aristotelian reading preserves objects---the electron is a genuine entity---but denies that objects are fully actual with respect to all their properties prior to measurement.

\medskip

This is a form of \textit{potentiality realism}: realism about both objects and their potentialities, with actuality restricted to what has been, or is being, measured. Objects are real; their potentialities are real; their actualities are contingent on measurement interactions.

\medskip

\TODO{Engage with Esfeld and Deckert's minimalist ontology \citep{esfeld_2017_minimalist}: points in a structure with no intrinsic properties. Argue potentiality realism is distinct: it accepts intrinsic properties \textit{in potentia}, not \textit{in actu}.}

\medskip

The distinction from Esfeld and Deckert's minimalist ontology
\cite{esfeld_2017_minimalist} is worth making explicit. Their
account posits structureless points whose only properties are
relational---there are no intrinsic properties at all, even
potential ones. Potentiality realism differs: it accepts that
objects have intrinsic properties, but holds that those properties
are possessed \textit{in potentia} prior to measurement rather than
\textit{in actu}. The electron has a real dispositional tendency
to be found in some region of space; that tendency is intrinsic
to it even before any measurement actualises it. This is a
genuinely distinct ontological position---neither the full
structural elimination of Esfeld and Deckert, nor the classical
picture of fully actual intrinsic properties at every instant.
...
The Wigner's friend paradox, discussed in Section~\ref{sec:wigner},
marks the boundary of what the present account can deliver: a fully
satisfactory potentiality realism must eventually specify the
physical conditions under which actualisation occurs and for whom,
a task we leave for further work.
